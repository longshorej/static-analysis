...
object evalSpec extends org.specs2.mutable.Specification {
  ...
  "GtExp #1" >> { eval(GtExp(IntExp(0), IntExp(0))) mustEqual 0 }
  "GtExp #2" >> { eval(GtExp(IntExp(1), IntExp(0))) mustEqual 1 }
  "GtExp #3" >> { eval(GtExp(IntExp(-1), IntExp(-5))) mustEqual 1 }

  "IfExp #1" >> { eval(IfExp(IntExp(0), IntExp(0), IntExp(1))) mustEqual 1 }
  "IfExp #2" >> { eval(IfExp(IntExp(1), IntExp(50), IntExp(51))) mustEqual 50 }
}
